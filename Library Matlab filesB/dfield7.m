...
%  Last modified: April 2, 2003
  ...
  if strcmp(action,'initialize')
    ...
    % CB: had to modify to get to work w/ 2015B release of Matlab (legacy
    % issue)
    %dfset = figure('name','dfield7 Setup','numb','off',...
	%	   'tag','dfield7','visible','off',...
	%	   'user',ud);
    
    dfset = figure('name','dfield7 Setup',...
		   'tag','dfield7','visible','off',...
		   'user',ud);
  ...
  meproceed = uimenu(mefile,'label','Proceed','call',...
    ...
    meadd = uimenu(sysmenu,'label','Add current equation to the gallery',...
		   'call','dfield7(''addgall'');','tag','add system');
    sep = 'on';
  ...
  elseif strcmp(action,'proceed')
    ...
    if any(isnan(WINvect))
      errmsg = ['One of the entries defining the display window ',...
		'is not a number.'];
      fprintf('\a')
      errordlg(errmsg,'dfield7 error','on');
      return
    end
    xstr = sud.c.xname;
    if isempty(xstr)
	...
	if ~isempty(pn)
	  if isempty(pv) 
	    ...
	    xderivstr = dfield7('paraeval',pn,pv,derivstr);
	...
	return
      ...
      % Find dfield7 Display if it exists.	
      % If dfield7 Display exists, update it.  If it does not build it.
    ...
    % CB: had to modify to get to work w/ 2015B release of Matlab (legacy
    % issue)
% 	dfdisp = figure('name','dfield7 Display',...
% 			'numb','off','interrupt','on',...
% 			'visible','off','tag','dfield7');
        dfdisp = figure('name','dfield7 Display',...
			'interrupt','on',...
			'visible','off','tag','dfield7');
	dfield7('figdefault',dfdisp);
	dud = get(dfdisp,'user');	
	dud.syst = sud.c;
	dud.settings = sud.settings;
	fs = dud.fontsize;
	...
	merestart = uimenu(fmenu,'label','Restart dfield7','call',...
			   'dfield7(''restart'')','separator','on','pos',8);
	
	mequit = uimenu(fmenu,'label','Quit dfield7',...
			'call','dfield7(''quit'')','separator','on');
  ...
  elseif strcmp(action,'solvset')
    ...
    setfig = figure('name','dfield7 Solver settings','numb','off',...
		    'tag','dfield7','vis','off');
    
    dfield7('figdefault',setfig);
  ...
  elseif strcmp(action,'settings')
    ...
    % up the dfield7 Settings window, which allows the user to interactively
    ...
    setfig = findobj('name','dfield7 Window settings');
    if ~isempty(setfig)
      close(setfig);
    end
    setfig = figure('name','dfield7 Window settings',...
		    'numb','off',...
		    'tag','dfield7','vis','off');
  ...
  elseif strcmp(action,'setchange')
    ...
    % on the dfield7 Settings window.
  ...
  elseif strcmp(action,'print')
    ...
    nstr = get(dud.notice,'string');
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Preparing to print the dfield7 Display Window.  Please be patient.';
    set(dud.notice,'string',nstr);
    
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Printing the dfield7 Display Window.';
    set(dud.notice,'string',nstr);
  ...
  elseif strcmp(action,'level')
    ...
    xname(find(abs(xname)==92))=[];
    dflevel = findobj('name','dfield7 Level sets');
    if ~isempty(dflevel)
      delete(dflevel)
    end
    dflevel = figure('name','dfield7 Level sets',...
		     'vis','off',...
		     'numb','off','tag','dfield7');
    
    dfield7('figdefault',dflevel);
    hhsetup = get(0,'showhiddenhandles');
  ...
  elseif strcmp(action,'levcomp')
	...
	if isempty(pv) 
	  ...
	  pv(find(abs(pv)==32))=[];
	  lfcn = dfield7('paraeval',pn,pv,lfcn);
	end
      end
    end
    l = length(lfcn);
  ...
  elseif strcmp(action,'showbar')
    ...
    if strcmp(state,'figure')
	  ...
	  'clickedcallback',['dfield7(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['dfield7(''zoomin'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Print'),...
	  'clickedcallback','dfield7(''print'');');
  ...
  elseif strcmp(action,'figdefault')
    ...
    fig = input1;
    set(fig,'CloseRequestFcn','dfield7(''closefcn'')');
    dfset = findobj('name','dfield7 Setup');
    sud = get(dfset,'user');
    ud = get(fig,'user');
    ud.ssize = sud.ssize;
  ...
  elseif strcmp(action,'closefcn')
    ...
    fig = gcf;  
    name = get(fig,'name');
    if strcmp(name,'dfield7 Setup') | strcmp(name,'dfield7 Display')
      quest = ['Closing this window will cause all dfield7 ',...
	       'windows to close, and dfield7 will stop.  ',...
	       'Do you want to quit dfield7?'];
      butt = questdlg(quest,'Quit dfield7?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
	dfield7('quit');
      end
    elseif strcmp(name,'dfield7 Linearization')
      dud = get(fig,'user');
      fcn = dud.function;
	if (exist(fcn)==2) delete([fcn,'.m']);end    
  ...
  elseif strcmp(action,'confused')
    ...
    tstring = 'dfield7 is totally confused';
    qstring = {['You will have to restart dfield7 from '...
		...
		'it would be best to do nothing now.'];
	       'What do you want to do?'};
    bstr1 = 'Quit and restart dfield7.';
    bstr2 = 'Just quit dfield7.';
    bstr3 = 'Do nothing.';
    answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
    if strcmp(answer,bstr1)
      delete(findobj('tag','dfield7'));
      dfield7;return
  ...
  elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
	...
	errmsg = 'This is not a dfield7 file.';
	fprintf('\a')
	errordlg(errmsg,'dfield7 error','on');
	return
      end
    ...
    for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = dfield7('addgall',newsyst);
      if ignore == -1;
...
% position window working while using ginput.

function uistate = dfuisuspend(fig)
...
function [out1,out2,out3] = dfginput(arg1)
