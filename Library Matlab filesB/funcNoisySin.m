...
% *** funcNoisySIN.m ***       2018.03.07  (updated 2020.09.17)     C. Bergevin

% Function to make a noisy sinusoidal waveform (e.g., to be used for
% testing inter-waveform correlation analysis codes; this sort of wveform
% can serve as "ground truth"). Allows for both additive noise as well as a
% Brownian noise envelope (w/ user-specified params.), both of which can
% independently be "frozen". Can also allow for "fast" noisy inst. phase
% (which amounts to a frequency modulation)
% ---
...
% >>> Phase noise
% NOTE (re To Do): Totally rewrote, so I need to update this list directly below
...
if (~isfield(P,'pF')), P.pFB= 0; end  % boolean to freeze phase (Brownian) noise {0}
if (~isfield(P,'pSeed')), P.pBSeed= 14; end  % seed ID for Brownian FM phase noise (reqs. P.nF=1)
